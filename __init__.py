...
def tomorrowDateStart():
    tomorrowStart = datetime.datetime.utcnow().replace(hour=00, minute=00, second=00)
...
def otherDateStart(until):
    otherDayStart = datetime.datetime.utcnow().replace(hour=00, minute=00, second=00)
...
def newDate(day,hours,minutes):
    newDate = datetime.datetime.utcnow().replace(hour=hours, minute=minutes, second=00)
...
class GoogleCalendarSkill(MycroftSkill):
    ...
    def eventJson(self,attendees_string,summary,start,end):
        ...
        lreadme = os.popen(addCalendar_cmd).read()
        print (lreadme)
        return msg
